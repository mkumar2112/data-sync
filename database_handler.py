...
def escape_name(db_type, name):
    if not name:  # Handle empty or None names
        raise ValueError("The name to escape cannot be None or empty.")
    
    if db_type == "mysql":
        # For MySQL, escape with backticks
        return f"`{name}`"
    elif db_type == "postgres":
        # For PostgreSQL, escape with double quotes
        return f'"{name}"'
    elif db_type == "mongo":
        # For MongoDB, no special escaping needed
        return name
    else:
...
class connectsql:
    ...
    def setup_connection(self, db_type, host=None,  user=None, password=None, database=None, url=None):
        """
        Set up the connection based on the database type and provided connection details.
        """
        try:
            if db_type == 'mysql':
                ...
                default_database = database if database else 'postgres'
                self.connection_details = {
                    'host': host,
                    'user': user,
                    'password': password,
        ...
        try:
            ...
            if self.db_exists(db_type, db_name):
                ...
                if db_type == 'mysql':
                    ...
                    # Create a new connection directly to the target postgres database
                    self.conn.close()
                ...
                # # print('DB is not created , creating db ----------------')
                is_created, message = self.create_db(db_type, db_name)
                if is_created:
                    # Database created successfully, now try to connect
                    return self.connectdb(db_type, db_name)
                else:
                    return False, f"Failed to create database {db_name}: {message}"
        except Exception as e:
            return False, str(e)

    def db_exists(self, db_type, db_name):
        """
        ...
        try:
            if db_type == 'mysql':
                ...
                db_exit = bool(cursor.fetchone())  # If a database is found, fetchone() will not be None
            elif db_type == 'postgres':
                cursor = self.conn.cursor()
    ...
    def create_db(self, db_type, db_name):
        ...
        try:
            if db_type == 'mysql' :
                ...
                cursor.close()
                # Reset isolation level (optional)
                ...
                # # print(f"MongoDB database {db_name} created.")
                return True, f"Database {db_name} created."
            else:
                raise ValueError("Unsupported database type.")
        except Exception as e:
    ...
    def close_connection(self):
        ...
        try:
            if self.conn:
                if isinstance(self.conn, (mysql.connector.MySQLConnection, psycopg2.extensions.connection)):
                    self.conn.close()
                elif isinstance(self.conn, MongoClient):
                    self.conn.close()
                self.conn = None
